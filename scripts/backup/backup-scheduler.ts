...
import { DatabaseBackup, BackupConfig } from './database-backup'
...
class BackupScheduler {
  ...
  generateHealthReport(): {
    ...
    const successRate = recentBackups.length > 0 ? (successfulBackups / recentBackups.length) * 100 : 0

    // Calculate average backup size
    const successfulBackupsWithSize = recentBackups.filter(b => b.success && b.fileSize)
    const averageSize = successfulBackupsWithSize.length > 0
      ? successfulBackupsWithSize.reduce((sum, b) => sum + (b.fileSize || 0), 0) / successfulBackupsWithSize.length
      : 0
    ...
    if (lastSuccessful) {
      const daysSinceLastSuccess = (Date.now() - new Date(lastSuccessful.timestamp).getTime()) / (1000 * 60 * 60 * 24)
      if (daysSinceLastSuccess > 7) {
        overall = 'critical'
        issues.push(`No successful backup in ${Math.floor(daysSinceLastSuccess)} days`)
      } else if (daysSinceLastSuccess > 2) {
        if (overall !== 'critical') overall = 'warning'
        issues.push(`Last successful backup was ${Math.floor(daysSinceLastSuccess)} days ago`)
      }
    } else {
      overall = 'critical'
...
async function main() {
  ...
  const scheduleConfig: ScheduleConfig = {
    ...
    backupConfig: {
      ...
      cloudStorage: process.env.BACKUP_CLOUD_PROVIDER ? {
        provider: process.env.BACKUP_CLOUD_PROVIDER as 'aws' | 'azure' | 'gcp',
  ...
  switch (command) {
    ...
    case 'health':
      ...
      const healthEmoji = health.overall === 'healthy' ? '✅' : health.overall === 'warning' ? '⚠️' : '❌'
      console.log(`${healthEmoji} Backup Health: ${health.overall.toUpperCase()}`)
      console.log(`  Success Rate: ${health.recentSuccessRate.toFixed(1)}%`)
      console.log(`  Average Size: ${(health.averageBackupSize / 1024 / 1024).toFixed(1)} MB`)
    ...
    default:
      ...
      console.log('  BACKUP_SCHEDULE_ENABLED - Enable/disable scheduler (default: true)')
