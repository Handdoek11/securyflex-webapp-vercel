...
const testOpdrachtData = {
  ...
  postcode: '1101AX',
  startDatum: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 16), // 1 week from now
  eindDatum: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000 + 10 * 60 * 60 * 1000).toISOString().slice(0, 16), // +10 hours
  uurloon: 28.50,
  aantalPersonen: 8,
...
async function createTestUser(page: Page) {
  ...
  await page.getByLabel(/wachtwoord/i).first().fill(testBedrijf.password);
...
test.describe('Bedrijf Complete User Flow', () => {
  test('should complete bedrijf registration and onboarding flow', async ({ page }) => {
    ...
    await expect(page.getByText(/welkom.*test beveiligingsbedrijf/i)).toBeVisible();
  ...
  test('should create and manage opdrachten as opdrachtgever', async ({ page }) => {
    // Login
    ...
    await page.getByLabel(/aantal personen/i).fill(testOpdrachtData.aantalPersonen.toString());
    ...
    await expect(page.getByText(`${testOpdrachtData.aantalPersonen} personen`)).toBeVisible();
  });
  ...
  test('should view and understand dashboard statistics', async ({ page }) => {
    ...
    // Test role switching
    await page.getByRole('button', { name: /leverancier.*opdrachtgever/i }).click();
  ...
  test('should handle error scenarios gracefully', async ({ page }) => {
    ...
    await expect(page.getByText(/fout.*laden/i)).toBeVisible({ timeout: 10000 });
  ...
  test('should validate form inputs correctly', async ({ page }) => {
    ...
    await expect(page.getByText(/startdatum mag niet in het verleden/i)).toBeVisible();
  });
  ...
  test('should work offline and handle reconnection', async ({ page, context }) => {
    // Login first
    await loginAsBedrijf(page);
...
test.describe('Bedrijf Advanced Workflows', () => {
  test('should handle bulk operations on opdrachten', async ({ page }) => {
    ...
    if (checkboxCount > 0) {
      ...
      await expect(page.getByRole('button', { name: /bulk.*acties/i })).toBeVisible();

      // Test bulk status change
      await page.getByRole('button', { name: /bulk.*acties/i }).click();
      await page.getByRole('menuitem', { name: /status wijzigen/i }).click();

      await expect(page.getByText(/status wijzigen voor.*opdrachten/i)).toBeVisible();
    }
  });
  ...
  test('should handle real-time updates', async ({ page, context }) => {
    ...
    await expect(page2.getByText('Real-time Test Opdracht')).toBeVisible({ timeout: 10000 });
  });
});
