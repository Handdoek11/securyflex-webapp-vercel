...
test.describe('Accessibility Tests', () => {
  ...
  test('should have no accessibility violations on ZZP dashboard', async ({ page }) => {
  ...
  test('should have proper ARIA labels and roles', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(buttonCount, 5); i++) {
      ...
      expect(hasAriaLabel || (hasVisibleText && hasVisibleText.trim().length > 0)).toBeTruthy()
    }
  })
  ...
  test('should have proper error handling for screen readers', async ({ page }) => {
    ...
    expect(hasAriaInvalid === 'true' || hasAriaDescribedBy !== null).toBeTruthy()
  })
