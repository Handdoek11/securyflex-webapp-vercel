...
test.describe("ZZP User Flow", () => {
  ...
  test.describe("Registration and Onboarding", () => {
    test("should complete ZZP registration flow", async () => {
      ...
      await expect(
        page.getByRole("heading", { name: /registreren/i }),
      ).toBeVisible();
  ...
  test.describe("Dashboard Navigation", () => {
    test("should display ZZP dashboard with key sections", async () => {
      ...
      await expect(
        page.getByRole("link", { name: /opdrachten/i }),
      ).toBeVisible();
      await expect(page.getByRole("link", { name: /werkuren/i })).toBeVisible();
      await expect(
        page.getByRole("link", { name: /sollicitaties/i }),
      ).toBeVisible();
      await expect(
        page.getByRole("link", { name: /betalingen/i }),
      ).toBeVisible();
  ...
  test.describe("Job Application Flow", () => {
    ...
    test("should view job details and apply for job", async () => {
      ...
      await page
        .getByLabel(/motivatie/i)
        .fill(
          "Ik ben zeer geïnteresseerd in deze opdracht en heb ervaring met beveiliging.",
        );

      // Submit application
      await page
        .getByRole("button", { name: /sollicitatie versturen/i })
        .click();
  ...
  test.describe("Work Hours Tracking", () => {
    test.skip("should start and track work hours with GPS", async () => {
      ...
      await expect(
        page.getByRole("button", { name: /dienst beëindigen/i }),
      ).toBeVisible();
    });
  ...
  test.describe("Payment Management", () => {
    ...
    test("should request payment for completed hours", async () => {
      ...
      if (await requestButton.isVisible()) {
        ...
        await page
          .getByLabel(/omschrijving/i)
          .fill("Beveiligingswerkzaamheden uitgevoerd");
        ...
        // Should show success message
        await expect(
          page.getByText(/betalingsaanvraag verstuurd/i),
        ).toBeVisible();
      }
    });
  });
  ...
  test.describe("Real-time Features", () => {
    test("should receive real-time notifications", async () => {
      ...
      await expect(page.getByText(/sollicitatie is geaccepteerd/i)).toBeVisible(
        { timeout: 5000 },
      );
    });
    ...
    test("should update data in real-time", async () => {
      ...
      await expect(page.getByText(/geaccepteerd/i)).toBeVisible({
        timeout: 5000,
      });
    });
  });
  ...
  test.describe("Error Handling and Edge Cases", () => {
    ...
    test("should validate form inputs correctly", async () => {
      ...
      await expect(
        page.getByText(/telefoonnummer is verplicht/i),
      ).toBeVisible();
    });
