...
test.describe("API Integration Tests", () => {
  ...
  test.describe("Job Management API", () => {
    ...
    test("should apply for job", async () => {
      ...
      const response = await apiContext.post(`/api/jobs/${testJobId}/apply`, {
        ...
        data: {
          motivation:
            "Ik ben zeer geïnteresseerd in deze beveiligingsopdracht en heb ruime ervaring.",
  ...
  test.describe("Payment API", () => {
    ...
    test("should request payment for completed hours", async () => {
      ...
      expect(data.paymentRequest.amount).toBe(150.0);
    });
  });
  ...
  test.describe("Data Consistency", () => {
    test("should maintain data consistency across operations", async () => {
      ...
      if (jobId) {
        const applyResponse = await apiContext.post(
        ...
        expect(fetchData.application.motivation).toBe(
          "Test application for consistency check",
        );
      }
    });
