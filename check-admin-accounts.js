...
async function checkAdminAccounts() {
  try {
    ...
    console.log('\n🎯 Summary:');
    const stefExists = await prisma.user.findUnique({ where: { email: 'stef@securyflex.com' } });
    const robertExists = await prisma.user.findUnique({ where: { email: 'robert@securyflex.com' } });

    console.log(`   Stef account: ${stefExists ? '✅ Exists' : '❌ Missing'}`);
    console.log(`   Robert account: ${robertExists ? '✅ Exists' : '❌ Missing'}`);
