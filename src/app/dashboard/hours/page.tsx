...
export default function HoursPage() {
  ...
  const fetchHours = async () => {
    ...
    try {
      ...
      toast.error("Netwerkfout bij laden van uren");
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitWeek = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success("Week ingediend voor goedkeuring");
        await fetchHours(); // Refresh data
      } else {
        toast.error(data.error || "Kon week niet indienen");
      }
    } catch (error) {
      console.error("Week submission error:", error);
      toast.error("Netwerkfout bij indienen week");
    } finally {
