...
import { Calendar, Clock, MapPin, Phone, CheckCircle, XCircle, AlertCircle, Navigation, Star, LogOut, LogIn, Loader2 } from "lucide-react";
...
function getStatusBadge(status: string) {
  switch (status) {
    case "BEVESTIGD":
      return <Badge className="bg-green-500 hover:bg-green-600">Bevestigd</Badge>;
    case "WACHTEN":
...
export default function ShiftsPage() {
  ...
  const [shifts, setShifts] = useState({ aankomend: [], actief: [], historie: [] });
  ...
  const fetchShifts = async () => {
    ...
    try {
      ...
      if (data.success) {
        setShifts(data.data.shifts || { aankomend: [], actief: [], historie: [] });
  ...
  const handleClockIn = async (shiftId: string, location: { lat: number; lng: number; accuracy: number }) => {
  ...
  const handleClockOut = async (werkuurId: string, location: { lat: number; lng: number; accuracy: number }) => {
            ...
            <TabsContent value="aankomend" className="space-y-4">
              {shifts.aankomend.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold mb-2">Geen aankomende shifts</h3>
                    ...
                    <Card key={shift.id} className="p-4 space-y-4">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground mb-2">{shift.company}</p>
                          {getStatusBadge(shift.status)}
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-green-600">€{shift.hourlyRate?.toFixed(2)}/uur</p>
                          <p className="text-xs text-muted-foreground">~€{shift.estimatedEarnings?.toFixed(2)}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span>{shiftDate.toLocaleDateString('nl-NL', {
                            weekday: 'short',
                            day: 'numeric',
                            month: 'short'
                          })}</span>
                      ...
                      {shift.instructions && (
                        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                          <p className="text-sm font-medium text-blue-800 dark:text-blue-200 mb-1">Instructies:</p>
                          <p className="text-sm text-blue-700 dark:text-blue-300">{shift.instructions}</p>
                        </div>
                      )}

                      <div className="flex items-center gap-2 text-sm">
                        <Phone className="h-4 w-4 text-muted-foreground" />
                        <span>{shift.contactPerson} - {shift.contactPhone}</span>
                      </div>

                      <div className="flex gap-2">
                        {shift.canClockIn ? (
                          <GPSTracker
                            onLocationUpdate={(location) => handleClockIn(shift.id, location)}
                            onError={(error) => toast.error(`GPS fout: ${error}`)}
            ...
            <TabsContent value="actief" className="space-y-4">
              {shifts.actief.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold mb-2">Geen actieve shifts</h3>
                  <p className="text-muted-foreground mb-4">
                    Je bent momenteel niet ingecheckt voor een shift.
                  </p>
                  <Button onClick={() => setActiveTab("aankomend")} variant="outline">
                ...
                shifts.actief.map((shift: any) => {
                  ...
                  return (
                    <Card key={shift.id} className="p-4 space-y-4 border-green-200 bg-green-50/50 dark:border-green-800 dark:bg-green-900/20">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <h3 className="font-semibold text-lg mb-1">{shift.title}</h3>
                          <p className="text-sm text-muted-foreground mb-2">{shift.company}</p>
                        ...
                        <div className="text-right">
                          <p className="text-sm text-muted-foreground">Ingecheckt om</p>
                          <p className="font-semibold">{shift.checkedInAt}</p>
                          <p className="text-xs text-muted-foreground mt-1">~{shift.hoursWorked?.toFixed(1)}u gewerkt</p>
                        </div>
                      </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span>{
                            shiftDate.toDateString() === new Date().toDateString()
                              ? "Vandaag"
                              : shiftDate.toLocaleDateString('nl-NL', { weekday: 'short', day: 'numeric', month: 'short' })
                          }</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Clock className="h-4 w-4 text-muted-foreground" />
                      ...
                      <div className="p-3 bg-green-100 dark:bg-green-900/30 rounded-lg">
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-green-800 dark:text-green-200">Huidige verdiensten:</span>
                          <span className="font-semibold text-green-900 dark:text-green-100">~€{shift.currentEarnings?.toFixed(2) || '0.00'}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <ConfirmDialog
                          trigger={
                            <GPSTracker
                              onLocationUpdate={(location) => handleClockOut(shift.werkuurId, location)}
                              onError={(error) => toast.error(`GPS fout: ${error}`)}
            ...
            <TabsContent value="historie" className="space-y-4">
              {shifts.historie.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold mb-2">Nog geen voltooide shifts</h3>
                  <p className="text-muted-foreground mb-4">
                  ...
                  return (
                    <Card key={shift.id} className="p-4 space-y-4">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                          <p className="font-semibold text-green-600">€{shift.totalEarned?.toFixed(2) || '0.00'}</p>
                          <p className="text-sm text-muted-foreground">{shift.hoursWorked?.toFixed(1) || 0} uur</p>
                          {shift.paymentStatus && (
                            <Badge variant={shift.paymentStatus === "PAID" ? "default" : "secondary"} className="text-xs mt-1">
                              {shift.paymentStatus === "PAID" ? "Betaald" : "In behandeling"}
                            </Badge>
                          )}
                        </div>
                      </div>
                      ...
                      {shift.rating && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-muted-foreground">({shift.rating}/5)</span>
                        </div>
                      )}
