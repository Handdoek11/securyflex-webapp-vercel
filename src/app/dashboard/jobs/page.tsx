...
import { Search, MapPin, Filter, Heart, Clock, Euro, Briefcase, CheckCircle, Users, AlertCircle } from "lucide-react";
...
export default function JobsPage() {
  ...
  }, [searchQuery, selectedFilter, session, activeTab]);
  ...
  const toggleFavorite = async (jobId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.success(isFavorited ? "Verwijderd uit favorieten" : "Toegevoegd aan favorieten");
      }
    } catch (error) {
      // Revert on failure
      setFavorites(prev =>
      ...
      <div className="p-4 space-y-4">
              ...
              return (
                <Card key={job.id} className="overflow-hidden">
                  ...
                  <div className="relative h-48 w-full">
                    <Image
                      src={job.image || "/images/jobs/default-beveiliging.jpg"}
                      alt={job.title}
                      fill
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    {job.applicationStatus && (
                      <Badge variant={
                        job.applicationStatus === "ACCEPTED" ? "default" :
                        job.applicationStatus === "REJECTED" ? "destructive" : "secondary"
                      } className="w-fit">
                    ...
                    <div className="flex gap-2 pt-2">
                      {job.applicationStatus === "ACCEPTED" ? (
                        ...
                        <>
                          ...
                          >
                            {job.spotsRemaining === 0 ? "Vol" : "Solliciteren"}
                          </Button>
                          <Button
                            variant="outline"
                            size="sm"
        ...
        {!loadingJobs && !error && filteredJobs.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">Geen jobs gevonden</h3>
          ...
          <TabsContent value="applications" className="space-y-4 mt-4">
            {loadingApplications ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">Nog geen sollicitaties</h3>
                <p className="text-muted-foreground mb-4">
                ...
                {applications.map((application) => (
                  <Card key={application.id} className="p-4">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <Badge variant={
                        ...
                        {application.status === "ACCEPTED" ? "Geaccepteerd" :
                         ...
                         application.status === "PENDING" ? "In behandeling" : "In review"}
                      </Badge>
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground mb-3">
                      ...
                      <div className="flex items-center gap-1">
                        <Clock className="h-3 w-3" />
                        <span>{new Date(application.job.startDate).toLocaleDateString("nl-NL")}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Users className="h-3 w-3" />
                        <span>{application.job.totalApplicants} sollicitanten</span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {application.status === "ACCEPTED" ? "Bekijk in Shifts" : "Bekijk Details"}
                      </Button>
                      {application.status === "PENDING" && (
                        <ConfirmDialog
                          ...
                          variant="destructive"
                          onConfirm={() => withdrawApplication(application.jobId)}
                        />
                      )}
                    </div>
                  </Card>
