  ...
  aankomend: [
    {
      ...
      status: "BEVESTIGD",
      instructions: "Melden bij hoofdingang om 17:45. Zwarte kleding en oordopjes meebrengen.",
  ...
  try {
    ...
    try {
      ...
      for (const app of applications) {
        ...
        const activeWorkHour = job.werkuren.find(wh => !wh.eindTijd && wh.startTijd);
        ...
        const totalHours = completedWorkHours.reduce((sum, wh) => sum + (wh.totaleUren || 0), 0);
        const totalEarnings = completedWorkHours.reduce((sum, wh) => sum + (wh.nettoBedrag || 0), 0);

        const shift = {
          id: job.id,
          title: job.titel,
          description: job.beschrijving,
          company: job.opdrachtgever?.bedrijfsnaam || job.bedrijf?.bedrijfsnaam || "Onbekend",
          ...
          contactPerson: job.opdrachtgever?.contactpersoon || job.bedrijf?.contactpersoon || "Contactpersoon",
          contactPhone: job.opdrachtgever?.telefoonnummer || job.bedrijf?.telefoonnummer || "Niet beschikbaar",
          address: job.locatie,
          instructions: job.instructies || null,
          estimatedEarnings: Math.round(((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60)) * Number(job.uurtarief) * 0.88),
        ...
        if (activeWorkHour) {
          ...
          shifts.actief.push({
            ...shift,
            status: "ACTIEF",
            checkedInAt: activeWorkHour.startTijd.toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit" }),
            werkuurId: activeWorkHour.id,
            currentEarnings: (now.getTime() - activeWorkHour.startTijd.getTime()) / (1000 * 60 * 60) * Number(job.uurtarief) * 0.88
          });
        } else if (endDate < now) {
          // Completed shifts
          ...
          // Upcoming shifts
          const canClockIn = (startDate.getTime() - now.getTime()) <= 30 * 60 * 1000; // 30 minutes before
    ...
    const stats = {
      totalShifts: shifts.aankomend.length + shifts.actief.length + shifts.historie.length,
      ...
      averageRating: shifts.historie.length > 0
        ? shifts.historie.reduce((sum, s) => sum + (s.rating || 0), 0) / shifts.historie.length
        : 0
    };
