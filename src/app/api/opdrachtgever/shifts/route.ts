...
const createShiftSchema = z.object({
  ...
  targetAudience: z.enum(["ALLEEN_BEDRIJVEN", "ALLEEN_ZZP", "BEIDEN"]).optional(),
  ...
  try {
    ...
    try {
      ...
      const transformedShifts = shifts.map(shift => ({
        ...
        needsAttention: shift.status === "OPEN" && new Date(shift.datum) < new Date(Date.now() + 24 * 60 * 60 * 1000),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const calculatedBudget = validatedData.budget ||
      (validatedData.uurtarief * validatedData.aantalBeveiligers *
       (parseInt(validatedData.eindTijd.split(':')[0]) - parseInt(validatedData.startTijd.split(':')[0])));
    ...
    try {
      ...
      console.log(`New shift created by opdrachtgever ${opdrachtgeverProfile.id}: ${newShift.id}`);
