...
async function createRegistrationAuditLog(
  ...
  try {
    const auditData = {
      ...
      ndNummer,
      previousStatus: 'NIET_GEREGISTREERD' as any,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { ndNummer, vervalDatum, documentUpload, confirmatie } = validation.data!;
    ...
    if (user.role === 'ZZP_BEVEILIGER') {
      ...
      return NextResponse.json(
        { error: 'Alleen ZZP beveiligers en beveiligingsbedrijven kunnen ND-nummer registreren' },
        { status: 403 }
      );
    }

    // Check if ND-nummer already exists in database (unique constraint)
    const existingNDNummer = await prisma.zZPProfile.findFirst({
      where: { ndNummer }
    ...
    const updateData = {
      ndNummer,
      ndNummerStatus: 'AANGEVRAAGD' as any, // Status will be verified later
    ...
    const daysUntilExpiry = Math.ceil((vervalDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Parse and validate request body
    const body = await request.json();
    const validation = validateApiRequest(ndNummerRegistrationSchema.partial(), body);
