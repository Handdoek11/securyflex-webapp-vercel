...
function calculateComplianceStatus(
  ...
  const now = new Date();
  const daysUntilExpiry = Math.ceil((vervalDatum.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
  ...
  if (isExpired || ndNummerStatus === 'INGETROKKEN' || ndNummerStatus === 'GESCHORST') {
    riskLevel = 'CRITICAL';
  ...
  try {
    ...
    const complianceData = [
      ...zzpProfiles.map(profile => ({
        ...
        ...calculateComplianceStatus(profile.ndNummerStatus, profile.ndNummerVervalDatum)
      })),
    ...
    const compliantProfiles = complianceData.filter(p => p.isCompliant).length;
    const expiredProfiles = complianceData.filter(p => p.isExpired).length;
    const expiringSoonProfiles = complianceData.filter(p => p.isExpiringSoon).length;
    const criticalRiskProfiles = complianceData.filter(p => p.riskLevel === 'CRITICAL').length;
    const highRiskProfiles = complianceData.filter(p => p.riskLevel === 'HIGH').length;
    ...
    return NextResponse.json({
      summary: {
        totalProfiles,
        compliantProfiles,
        complianceRate: totalProfiles > 0 ? Math.round((compliantProfiles / totalProfiles) * 100) : 0,
        expiredProfiles,
      ...
      recentActivity: recentAudits.map(audit => ({
        ...
        profileName: audit.profileType === 'ZZP' ?
          audit.zzpProfile?.user?.name :
          audit.bedrijfProfile?.bedrijfsnaam || audit.bedrijfProfile?.user?.name,
      ...
      alerts: complianceData
        ...
        .map(profile => ({
          ...
          ndNummer: profile.ndNummer,
          riskLevel: profile.riskLevel,
          issue: profile.isExpired ? 'ND-nummer verlopen' :
                 profile.isExpiringSoon ? `Verloopt over ${profile.daysUntilExpiry} dagen` :
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { profileIds, checkType, includeExpiringSoon, daysBeforeExpiry, includeInactiveProfiles } = validation.data!;
    ...
    if (!isAdmin) {
      ...
      const unauthorizedProfiles = profileIds.filter(id => !allowedProfileIds.includes(id));
    ...
    const zzpProfiles = await prisma.zZPProfile.findMany({
      where: {
        id: { in: profileIds },
        ...(includeInactiveProfiles ? {} : { ndNummerStatus: { not: 'NIET_GEREGISTREERD' } })
      },
      include: {
        user: {
          select: {
    ...
    const allProfiles = [
      ...
      ...bedrijfProfiles.map(p => ({ ...p, profileType: 'BEDRIJF' as const }))
    ];

    // Perform compliance checks
    const checkResults = allProfiles.map(profile => {
      const compliance = calculateComplianceStatus(profile.ndNummerStatus, profile.ndNummerVervalDatum);
      ...
      if (checkType === 'EXPIRY_CHECK' || checkType === 'FULL_AUDIT') {
        ...
        } else if (includeExpiringSoon && compliance.daysUntilExpiry !== null && compliance.daysUntilExpiry <= daysBeforeExpiry) {
          needsAttention = true;
      ...
      if (checkType === 'STATUS_VERIFICATION' || checkType === 'FULL_AUDIT') {
        if (profile.ndNummerStatus === 'GESCHORST' || profile.ndNummerStatus === 'INGETROKKEN') {
          needsAttention = true;
      ...
      return {
        ...
        recommendations: needsAttention ? [
          ...
          compliance.isExpiringSoon ? 'Plan vernieuwing binnen 30 dagen' : null,
          profile.ndNummerStatus === 'GESCHORST' ? 'Neem contact op met Justis' : null,
        ].filter(Boolean) : []
      };
    });
    ...
    const summary = {
      ...
      expiringSoon: checkResults.filter(r => r.compliance.isExpiringSoon).length,
      criticalIssues: checkResults.filter(r => r.compliance.riskLevel === 'CRITICAL').length,
      checkType,
    ...
    return NextResponse.json({
      ...
      nextActions: {
        scheduleNotifications: checkResults.filter(r => r.needsAttention).length > 0,
        requiresImmediateAction: checkResults.filter(r => r.compliance.riskLevel === 'CRITICAL').length > 0,
