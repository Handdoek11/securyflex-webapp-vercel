...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!bedrijfProfile) {
      return NextResponse.json(
        { success: false, error: "Alleen bedrijven kunnen dashboard stats bekijken" },
        { status: 403 }
      );
    }
    ...
    const [
      ...
      prisma.opdracht.findMany({
        where: {
          ...baseWhereClause,
          createdAt: {
            gte: new Date(now.getFullYear(), now.getMonth(), now.getDate() - 84) // 12 weeks
          }
        },
        select: {
          createdAt: true,
    ...
    const opdrachtGrowth = previousCount > 0
      ? ((currentCount - previousCount) / previousCount) * 100
      : currentCount > 0 ? 100 : 0;

    const currentRevenue = Number(currentStats._sum.uurtarief || 0) * Number(currentStats._sum.aantalBeveiligers || 0) * 8; // Estimate 8 hours
    const previousRevenue = Number(previousStats._sum.uurtarief || 0) * Number(previousStats._sum.aantalBeveiligers || 0) * 8;
    ...
    // Process team stats
    const teamPerformance = teamStats.reduce((acc, item) => {
      acc[item.status] = item._count.id;
      return acc;
    }, {} as Record<string, number>);

    // Calculate success rates
    const totalApplications = Object.values(teamPerformance).reduce((sum, count) => sum + count, 0);
    const successRate = totalApplications > 0
      ? ((teamPerformance.ACCEPTED || 0) / totalApplications) * 100
      : 0;

    // Process revenue timeline for chart
    const revenueTimeline = revenueData.reduce((acc, opdracht) => {
      const weekKey = getWeekKey(opdracht.createdAt);
      if (!acc[weekKey]) {
        acc[weekKey] = { week: weekKey, revenue: 0, opdrachten: 0 };
      }
      const estimated = Number(opdracht.uurtarief) * opdracht.aantalBeveiligers * 8;
      acc[weekKey].revenue += estimated;
      acc[weekKey].opdrachten += 1;
      return acc;
    }, {} as Record<string, { week: string; revenue: number; opdrachten: number }>);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        recentOpdrachten: recentOpdrachten.map(opdracht => ({
          ...
          isFullyStaffed: opdracht._count.sollicitaties >= opdracht.aantalBeveiligers
        })),
...
function generateInsights(data: {
  ...
  if (data.currentCount > data.previousCount) {
    ...
    insights.push({
      type: 'warning',
      title: 'Dalende vraag',
      message: 'Minder opdrachten dan vorige periode. Overweeg marketing activiteiten.'
    });
  }
  ...
  if (data.successRate > 80) {
    ...
    insights.push({
      ...
      message: 'Minder dan 50% acceptatie. Check of jullie tarieven/voorwaarden competitief zijn.'
    });
  }
  ...
  if (data.upcomingCount > 5) {
    ...
    insights.push({
      ...
      message: 'Overweeg om nieuwe opdrachten te zoeken of klanten te contacteren.'
    });
  }
