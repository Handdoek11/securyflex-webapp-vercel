...
export async function GET(request: NextRequest) {
  try {
    ...
    let shouldShowComplianceWarning = false;
    ...
    if (view === "available" && (user.zzpProfile || user.bedrijfProfile)) {
      ...
      shouldShowComplianceWarning = !ndNummerValid ||
                                   (ndNummerExpiry &&
                                    Math.ceil((new Date(ndNummerExpiry).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)) <= 30);
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            complianceWarning: {
              show: true,
              ndNummerStatus,
              message: !ndNummerStatus || ndNummerStatus === 'NIET_GEREGISTREERD'
    ...
    if (view === "available") {
      ...
      where.OR = [
        { creatorBedrijfId: user.bedrijfProfile.id, targetAudience: "EIGEN_TEAM" },
    ...
    const formattedOpdrachten = opdrachten.map(opdracht => ({
      ...
      creator: opdracht.creatorType === "OPDRACHTGEVER"
      ...
      stats: {
        ...
        openSpots: Math.max(0, opdracht.aantalBeveiligers - opdracht.assignments.filter(a => a.status === "CONFIRMED").length)
      }
    }));
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user?.opdrachtgever && !user?.bedrijfProfile) {
      return NextResponse.json(
        { success: false, error: "Only opdrachtgevers and bedrijven can create opdrachten" },
        { status: 403 }
      );
    }
    ...
    const opdracht = await prisma.opdracht.create({
      data: {
        ...
        directZZPAllowed: validatedData.directZZPAllowed ?? (validatedData.targetAudience !== "ALLEEN_BEDRIJVEN"),
    ...
    await broadcastOpdrachtEvent(
      ...
      {
        creatorType,
        creatorName: user.opdrachtgever?.bedrijfsnaam || user.bedrijfProfile?.bedrijfsnaam
      }
    );

    // Create notification for relevant users
    if (creatorType === "OPDRACHTGEVER" && validatedData.targetAudience !== "EIGEN_TEAM") {
