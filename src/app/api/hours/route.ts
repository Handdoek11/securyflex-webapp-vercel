...
export async function GET(request: NextRequest) {
  try {
    ...
    let timeEntries = [];
    let weeklyStats = { totalHours: 0, totalEarnings: 0, approvedEarnings: 0, pendingEarnings: 0 };
    ...
    try {
      ...
      timeEntries = workHours.map(wh => {
        ...
        return {
          ...
          company: wh.opdracht.opdrachtgever?.bedrijfsnaam || wh.opdracht.bedrijf?.bedrijfsnaam || "Onbekend",
          project: wh.opdracht.titel,
          startTime: wh.startTijd ? wh.startTijd.toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit" }) : "",
          endTime: wh.eindTijd ? wh.eindTijd.toLocaleTimeString("nl-NL", { hour: "2-digit", minute: "2-digit" }) : "",
      ...
      weeklyStats = {
        ...
        totalEarnings: timeEntries.reduce((sum, entry) => sum + entry.totalEarned, 0),
      ...
      console.log(`Found ${workHours.length} work hours from database for week ${startDate.toISOString().split('T')[0]}`);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        weekInfo: {
          ...
          endDate: weekEnd || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString()
        }
      }
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { opdrachtId, datum, startTijd, eindTijd, pauzetijd, opmerkingen } = body;
    ...
    const workMinutes = (end.getTime() - start.getTime()) / (1000 * 60) - (pauzetijd || 0);
