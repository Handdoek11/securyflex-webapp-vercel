...
export interface FinqleNotification {
  id: string;
  type: "payment_received" | "payment_failed" | "kyc_approved" | "kyc_rejected" | "payout_initiated";
  title: string;
  message: string;
  createdAt: string;
  read: boolean;
...
export function usePaymentRealtime(userId?: string) {
  ...
  const [paymentEvents, setPaymentEvents] = useState<Array<{
    type: string;
    data: any;
    timestamp: Date;
  }>>([]);
...
export function useBatchPayments() {
  ...
  const processBatchPayout = useCallback(async (paymentData: {
    zzpPayments: Array<{
      zzpProfileId: string;
      amount: number;
      description?: string;
...
export function useFinqleIntegration() {
  ...
  const connectToFinqle = useCallback(async (userData: {
    companyName?: string;
    taxId?: string;
    businessType?: string;
  }) => {
