...
export interface RealtimeUpdate {
  type: 'opdracht_created' | 'opdracht_updated' | 'payment_updated' | 'team_updated' | 'planning_updated';
  data: any;
  timestamp: string;
  userId?: string;
  bedrijfId?: string;
...
export function useTeamRealtime(bedrijfId?: string) {
  const [teamStatus, setTeamStatus] = useState<Map<string, {
    online: boolean;
    lastSeen: string;
  ...
  const getTeamMemberStatus = useCallback((memberId: string) => {
    return teamStatus.get(memberId) || {
      online: false,
...
export function usePaymentRealtime(bedrijfId?: string) {
  const [paymentUpdates, setPaymentUpdates] = useState<Array<{
    paymentId: string;
    status: string;
    amount?: number;
    timestamp: string;
  ...
  const getPaymentUpdate = useCallback((paymentId: string) => {
...
export function useDashboardRealtime(bedrijfId?: string) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribe(`bedrijf:${bedrijfId}:dashboard`, (update) => {
