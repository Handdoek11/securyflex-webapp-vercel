...
import { ErrorBoundary, useErrorHandler, retryWithBackoff, handleNetworkError, APIError, ValidationError, GPSError } from './error-boundary'
...
const ThrowValidationError = ({ shouldThrow = false }: { shouldThrow?: boolean }) => {
  if (shouldThrow) {
...
describe('ErrorBoundary', () => {
  ...
  it('should render default error fallback when error occurs', () => {
    ...
    expect(screen.getByRole('button', { name: /naar dashboard/i })).toBeInTheDocument()
  })
...
describe('GPS Error Boundary', () => {
  it('should render GPS-specific error fallback', () => {
    ...
    expect(screen.getByRole('button', { name: /gps probeer opnieuw/i })).toBeInTheDocument()
  })
})
...
describe('handleNetworkError', () => {
  ...
  it('should return offline message when no connection', () => {
    ...
    expect(message).toBe('Geen internetverbinding. Controleer je netwerk en probeer opnieuw.')
  })
  ...
  it('should handle fetch errors', () => {
    ...
    expect(message).toBe('Kan geen verbinding maken met de server. Probeer later opnieuw.')
  })
  ...
  it('should handle unknown errors', () => {
    ...
    expect(message).toBe('Er is een onverwachte fout opgetreden. Probeer opnieuw.')
  })
})
...
describe('retryWithBackoff', () => {
  ...
  it('should fail after max retries', async () => {
    ...
    await expect(retryWithBackoff(fn, { maxRetries: 2, baseDelay: 10 })).rejects.toThrow('Always fails')
