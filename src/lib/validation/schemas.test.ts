...
describe("Opdracht Creation Schema", () => {
  it("should validate complete opdracht data", () => {
    const validOpdracht = {
      ...
      beschrijving:
        "Beveiliging van een klassiek concert met ongeveer 500 bezoekers. Ervaring met evenement beveiliging vereist.",
      locatie: {
...
describe("Validation Helper Functions", () => {
  describe("validateApiRequest", () => {
    it("should sanitize string inputs when sanitize option is true", () => {
      ...
      const result = validateApiRequest(schema, maliciousData, {
        sanitize: true,
      });
    ...
    it("should allow partial validation", () => {
      ...
      const result = validateApiRequest(schema, partialData, {
        allowPartial: true,
      });
      expect(result.success).toBe(true);
    });
