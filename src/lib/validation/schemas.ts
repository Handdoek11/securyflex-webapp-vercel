...
export const baseValidations = {
  ...
  btwNumber: z.string().regex(btwRegex, "Ongeldig BTW nummer (bijv. NL123456789B01)"),
  postcode: z.string().regex(postcodeRegex, "Ongeldige postcode"),
  ndNummer: z.string().regex(ndNummerRegex, "Ongeldig ND-nummer (bijv. ND123456)"),
  password: z.string().min(8, "Wachtwoord moet minimaal 8 karakters bevatten"),
  name: z.string().min(2, "Naam moet minimaal 2 karakters bevatten"),
  hourlyRate: z.number().min(15, "Uurtarief moet minimaal €15 zijn").max(100, "Uurtarief kan niet hoger zijn dan €100"),
};
...
export const zzpProfileSchema = z.object({
  ...
  achternaam: z.string().min(2, "Achternaam moet minimaal 2 karakters bevatten"),
  ...
  adres: z.string().min(5, "Adres moet minimaal 5 karakters bevatten").optional(),
  postcode: baseValidations.postcode.optional(),
  plaats: z.string().min(2, "Plaats moet minimaal 2 karakters bevatten").optional(),

  // Professional info
  uurtarief: baseValidations.hourlyRate,
  specialisaties: z.array(z.string()).min(1, "Selecteer minimaal 1 specialisatie"),
  ...
  ervaring: z.number().min(0, "Ervaring kan niet negatief zijn").max(50, "Maximaal 50 jaar ervaring").optional(),
...
export const clockInSchema = z.object({
  ...
  opmerkingen: z.string().max(500, "Opmerkingen kunnen maximaal 500 karakters bevatten").optional(),
});
...
export const clockOutSchema = z.object({
  ...
  incidenten: z.array(z.object({
    type: z.enum(["ALARM", "INCIDENT", "NOODGEVAL", "OVERIG"]),
    beschrijving: z.string().min(10, "Beschrijving moet minimaal 10 karakters bevatten"),
    tijdstip: z.string(),
...
export const jobApplicationSchema = z.object({
  motivatie: z.string().min(20, "Motivatie moet minimaal 20 karakters bevatten").optional(),
...
export const bedrijfProfileSchema = z.object({
  bedrijfsnaam: z.string().min(2, "Bedrijfsnaam moet minimaal 2 karakters bevatten"),
  kvkNummer: baseValidations.kvkNumber,
  ...
  specialisaties: z.array(z.string()).min(1, "Selecteer minimaal 1 specialisatie"),
  werkgebied: z.array(z.string()).min(1, "Selecteer minimaal 1 werkgebied"),
  aantalMedewerkers: z.number().min(1, "Aantal medewerkers moet minimaal 1 zijn").max(1000),
  oprichtingsjaar: z.number().min(1900).max(new Date().getFullYear()),
  certificeringen: z.array(z.string()).optional(),
  bedrijfsstructuur: z.enum(["BV", "NV", "VOF", "EENMANSZAAK", "STICHTING", "ANDERS"]),
});

// Opdrachtgever Profile schema - aligned with Prisma schema
export const opdrachtgeverProfileSchema = z.object({
  bedrijfsnaam: z.string().min(2, "Bedrijfsnaam moet minimaal 2 karakters bevatten"), // Changed from organisatienaam to match Prisma
  contactpersoon: z.string().min(2, "Contactpersoon moet minimaal 2 karakters bevatten"),
  kvkNummer: baseValidations.kvkNumber.optional(), // Optional in Prisma
  email: baseValidations.email,
  telefoon: baseValidations.phone,
  website: z.string().url("Ongeldige website URL").optional(),
  adres: z.string().min(5, "Adres moet minimaal 5 karakters bevatten"),
  ...
  organisatietype: z.enum(["BEDRIJF", "OVERHEID", "ZIEKENHUIS", "SCHOOL", "EVENEMENT", "PARTICULIER", "ANDERS"]),
...
export const opdrachtCreateSchema = z.object({
  titel: z.string().min(5, "Titel moet minimaal 5 karakters bevatten").max(100, "Titel mag maximaal 100 karakters bevatten"),
  beschrijving: z.string().min(20, "Beschrijving moet minimaal 20 karakters bevatten").max(2000, "Beschrijving mag maximaal 2000 karakters bevatten"),
  locatie: z.object({
    adres: z.string().min(5, "Adres moet minimaal 5 karakters bevatten"),
    postcode: baseValidations.postcode,
    plaats: z.string().min(2, "Plaats moet minimaal 2 karakters bevatten"),
  ...
  startTijd: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, "Ongeldige tijd (HH:MM)"),
  eindTijd: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, "Ongeldige tijd (HH:MM)"),
  uurtarief: z.number().min(15, "Uurtarief moet minimaal €15 zijn").max(100, "Uurtarief mag maximaal €100 zijn"),
  aantalBeveiligers: z.number().min(1, "Minimaal 1 beveiliger").max(50, "Maximaal 50 beveiligers"),
  urgentie: z.enum(["LAAG", "NORMAAL", "HOOG", "SPOED"]),
  vereisten: z.array(z.string()).optional(),
  specialisaties: z.array(z.string()).min(1, "Selecteer minimaal 1 specialisatie"),
  targetAudience: z.enum(["ALLEEN_BEDRIJVEN", "ALLEEN_ZZP", "BEIDEN", "EIGEN_TEAM"]),
  directZZPAllowed: z.boolean(),
  automatischGoedkeuren: z.boolean().default(false),
  bijzonderheden: z.string().max(1000, "Bijzonderheden mogen maximaal 1000 karakters bevatten").optional(),
});
...
export const werkurenLogSchema = z.object({
  ...
  pauzes: z.array(z.object({
    start: z.string().datetime("Ongeldige pauze start tijd"),
    eind: z.string().datetime("Ongeldige pauze eind tijd"),
    reden: z.string().max(200, "Reden mag maximaal 200 karakters bevatten").optional(),
  })).optional(),
  locatieChecks: z.array(z.object({
  ...
  opmerkingen: z.string().max(1000, "Opmerkingen mogen maximaal 1000 karakters bevatten").optional(),
...
export const reviewSchema = z.object({
  ...
  rating: z.number().min(1, "Rating moet minimaal 1 zijn").max(5, "Rating mag maximaal 5 zijn"),
  ...
  opmerking: z.string().min(10, "Opmerking moet minimaal 10 karakters bevatten").max(1000, "Opmerking mag maximaal 1000 karakters bevatten"),
...
export const messageSchema = z.object({
  ...
  onderwerp: z.string().min(3, "Onderwerp moet minimaal 3 karakters bevatten").max(100, "Onderwerp mag maximaal 100 karakters bevatten"),
  bericht: z.string().min(5, "Bericht moet minimaal 5 karakters bevatten").max(2000, "Bericht mag maximaal 2000 karakters bevatten"),
  prioriteit: z.enum(["LAAG", "NORMAAL", "HOOG", "URGENT"]).default("NORMAAL"),
  type: z.enum(["PERSOONLIJK", "OPDRACHT", "SYSTEEM", "MELDING"]).default("PERSOONLIJK"),
...
export const paymentRequestSchema = z.object({
  ...
  bedrag: z.number().min(0.01, "Bedrag moet groter dan €0 zijn").max(10000, "Bedrag mag maximaal €10.000 zijn"),
  beschrijving: z.string().min(5, "Beschrijving moet minimaal 5 karakters bevatten").max(500, "Beschrijving mag maximaal 500 karakters bevatten"),
  verwachteBetaaldatum: z.string().min(10, "Verwachte betaaldatum is verplicht"),
  bijlagen: z.array(z.string().url("Ongeldige bijlage URL")).optional(),
});

// API Query parameters schema
export const queryParamsSchema = z.object({
  page: z.string().regex(/^\d+$/, "Pagina moet een getal zijn").transform(Number).refine(n => n > 0, "Pagina moet groter dan 0 zijn").optional(),
  limit: z.string().regex(/^\d+$/, "Limiet moet een getal zijn").transform(Number).refine(n => n > 0 && n <= 100, "Limiet tussen 1 en 100").optional(),
...
export const ndNummerRegistrationSchema = z.object({
  ...
  confirmatie: z.boolean().refine(val => val === true, "Bevestiging is verplicht"),
}).refine(data => {
...
export const ndNummerStatusUpdateSchema = z.object({
  ...
  reden: z.string().min(5, "Reden voor statuswijziging is verplicht").max(500, "Reden mag maximaal 500 karakters bevatten"),
  verificationSource: z.enum(["Justis API", "Manual", "Automated"]).default("Manual"),
  apiResponse: z.any().optional(), // For storing full API response
});

// ND-nummer renewal schema
export const ndNummerRenewalSchema = z.object({
  ndNummer: baseValidations.ndNummer,
  nieuweVervalDatum: z.string().datetime("Ongeldige nieuwe vervaldatum"),
  renewalDocuments: z.array(z.string().url("Ongeldige document URL")).min(1, "Minimaal 1 document vereist"),
  kostenbetaling: z.object({
    bedrag: z.number().min(500, "Minimaal €500 voor vernieuwing").max(1000, "Maximaal €1000 voor vernieuwing"),
...
export const ndNummerComplianceCheckSchema = z.object({
  profileIds: z.array(z.string()).min(1, "Minimaal 1 profiel ID vereist").max(100, "Maximaal 100 profielen per check"),
  checkType: z.enum(["EXPIRY_CHECK", "STATUS_VERIFICATION", "FULL_AUDIT"]),
  includeExpiringSoon: z.boolean().default(true),
  daysBeforeExpiry: z.number().min(1, "Minimaal 1 dag").max(365, "Maximaal 365 dagen").default(90),
...
export const ndNummerNotificationSchema = z.object({
  ...
  channels: z.array(z.enum(["EMAIL", "SMS", "PUSH", "IN_APP"])).min(1, "Minimaal 1 notificatie kanaal"),
...
export type OpdrachtgeverProfileData = z.infer<typeof opdrachtgeverProfileSchema>;
export type OpdrachtgeverDbProfileData = z.infer<typeof opdrachtgeverDbProfileSchema>;
...
// ND-nummer types
export type NDNummerRegistrationData = z.infer<typeof ndNummerRegistrationSchema>;
export type NDNummerVerificationData = z.infer<typeof ndNummerVerificationSchema>;
export type NDNummerStatusUpdateData = z.infer<typeof ndNummerStatusUpdateSchema>;
...
export type NDNummerComplianceCheckData = z.infer<typeof ndNummerComplianceCheckSchema>;
export type NDNummerNotificationData = z.infer<typeof ndNummerNotificationSchema>;
...
export function validatePartial<T>(schema: z.ZodSchema<T>, data: unknown): { success: boolean; errors?: Record<string, string> } {
  try {
    schema.parse(data);
    return { success: true };
  } catch (error) {
...
): { success: boolean; data?: T; errors?: Record<string, string>; error?: string } {
  try {
    const parseMethod = options.allowPartial ? schema.partial().parse : schema.parse;
...
export const apiValidationMiddleware = {
  ...
  validateBody: <T>(schema: z.ZodSchema<T>) =>
    (req: any): { success: boolean; data?: T; errors?: Record<string, string> } => {
