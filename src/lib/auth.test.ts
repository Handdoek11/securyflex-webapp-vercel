...
import { hashPassword, verifyPassword, requireAuth, requireRole, requireCompletedProfile } from './auth'
...
describe('Auth Utilities', () => {
  ...
  describe('verifyPassword', () => {
    ...
    it('should handle verification errors', async () => {
      ...
      await expect(verifyPassword('password123', 'hashed-password')).rejects.toThrow('Verification failed')
    })
  })
  ...
  describe('requireRole', () => {
    it('should return user when role matches', async () => {
      ...
      const result = await requireRole([UserRole.ZZP_BEVEILIGER, UserRole.BEDRIJF])
    ...
    it('should throw error when role does not match', async () => {
      ...
      await expect(requireRole([UserRole.BEDRIJF, UserRole.OPDRACHTGEVER])).rejects.toThrow('Insufficient permissions')
    })
    ...
    it('should throw error when not authenticated', async () => {
      ...
      await expect(requireRole([UserRole.ZZP_BEVEILIGER])).rejects.toThrow('Authentication required')
    })
  })
  ...
  describe('requireCompletedProfile', () => {
    ...
    it('should throw error when profile is not completed', async () => {
      ...
      await expect(requireCompletedProfile()).rejects.toThrow('Profile completion required')
    })
    ...
    it('should throw error when not authenticated', async () => {
      ...
      await expect(requireCompletedProfile()).rejects.toThrow('Authentication required')
    })
  })
