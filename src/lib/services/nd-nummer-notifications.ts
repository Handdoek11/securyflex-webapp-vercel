...
export function getNotificationTemplate(
  ...
  const templates: Record<NDNummerNotification['notificationType'], NotificationTemplate> = {
    EXPIRY_WARNING_90_DAYS: {
    ...
    STATUS_CHANGE: {
      ...
      priority: data.newStatus === 'GESCHORST' || data.newStatus === 'INGETROKKEN' ? 'CRITICAL' : 'MEDIUM'
    },
 ...
 */
export async function sendNotification(notification: NDNummerNotification): Promise<boolean> {
  try {
    // Get user profile information
    const profile = notification.profileType === 'ZZP'
    ...
    if (!profile) {
      console.error('Profile not found for notification:', notification.profileId);
      return false;
    }

    const userName = profile.user.name;
    const bedrijfsnaam = 'bedrijfsnaam' in profile ? profile.bedrijfsnaam : undefined;
...
export async function checkExpiringNDNummers(): Promise<void> {
  try {
    ...
    const allExpiringProfiles = [
      ...
      ...expiringBedrijven.map(p => ({ ...p, profileType: 'BEDRIJF' as const }))
    ];
    ...
    for (const profile of allExpiringProfiles) {
      ...
      const notification: NDNummerNotification = {
        ...
        notificationType,
        channels: urgency === 'CRITICAL' ? ['EMAIL', 'SMS', 'PUSH', 'IN_APP'] : ['EMAIL', 'IN_APP'],
        urgency,
    ...
    console.log(`ND-nummer expiry check completed. Processed ${allExpiringProfiles.length} expiring profiles and ${expiredZZPs.length + expiredBedrijven.length} expired profiles.`);
 ...
 * Helper functions
 */
function getInAppNotificationType(notificationType: NDNummerNotification['notificationType']) {
  const mapping = {
...
async function logNotificationSent(
  ...
  try {
    await prisma.nDNummerAuditLog.create({
      data: {
        profileType: notification.profileType,
        zzpProfileId: notification.profileType === 'ZZP' ? notification.profileId : undefined,
        bedrijfProfileId: notification.profileType === 'BEDRIJF' ? notification.profileId : undefined,
        ndNummer: notification.ndNummer,
