...
describe('Rate Limiting', () => {
  ...
  describe('checkRateLimit', () => {
    it('should return null when rate limit is not exceeded', async () => {
      const req = createMockRequest('http://localhost/api/test', { ip: '192.168.1.1' })
    ...
    it('should return rate limit response when limit is exceeded', async () => {
      const req = createMockRequest('http://localhost/api/test', { ip: '192.168.1.2' })
  ...
  describe('rateLimitMiddleware', () => {
    ...
    it('should apply rate limiting to API routes', async () => {
      const req = createMockRequest('http://localhost/api/users', { ip: '192.168.1.3' })
  ...
  describe('checkUserActionLimit', () => {
    ...
    it('should isolate rate limits between different users', async () => {
      ...
      const user1Result = await checkUserActionLimit(req, 'user-1', action, options)
      expect(user1Result).not.toBeNull()

      // User 2 should still be able to make requests
      const user2Result = await checkUserActionLimit(req, 'user-2', action, options)
    ...
    it('should isolate rate limits between different actions', async () => {
      ...
      const action1Result = await checkUserActionLimit(req, userId, 'action1', options)
      expect(action1Result).not.toBeNull()

      // Action 2 should still be available
      const action2Result = await checkUserActionLimit(req, userId, 'action2', options)
  ...
  describe('isTrustedSource', () => {
    it('should trust localhost IPs', () => {
      const req1 = createMockRequest('http://localhost/api/webhook', { ip: '127.0.0.1' })
      const req2 = createMockRequest('http://localhost/api/webhook', { ip: '::1' })
    ...
    it('should not trust external IPs', () => {
      const req = createMockRequest('http://localhost/api/webhook', { ip: '203.0.113.1' })
  ...
  describe('rateLimitWithTrustCheck', () => {
    it('should bypass rate limiting for trusted sources', async () => {
      const req = createMockRequest('http://localhost/api/webhook', { ip: '127.0.0.1' })
    ...
    it('should apply rate limiting for non-trusted sources', async () => {
      const req = createMockRequest('http://localhost/api/users', { ip: '203.0.113.1' })
  ...
  describe('addRateLimitHeaders', () => {
    it('should add rate limit headers to response', () => {
      ...
      expect(response.headers.set).toHaveBeenCalledWith('X-RateLimit-Limit', '100')
      expect(response.headers.set).toHaveBeenCalledWith('X-RateLimit-Remaining', '90')
