...
export class SecuryFlexMonitoring {
  ...
  static setUserContext(user: User) {
    Sentry.setUser({
      ...
      username: user.name,
      role: user.role,
      segment: SecuryFlexMonitoring.getUserSegment(user.role),
    });
    ...
    Sentry.setTag(
      "user.segment",
      SecuryFlexMonitoring.getUserSegment(user.role),
    );
  ...
  static trackRateLimit(
    ...
    Sentry.addBreadcrumb({
      ...
      data: {
        endpoint,
        ip: SecuryFlexMonitoring.anonymizeIP(ip),
        limit,
        remaining,
      },
  ...
  static setRequestContext(
    ...
    Sentry.setContext("request", {
      ...
      ip: ip ? SecuryFlexMonitoring.anonymizeIP(ip) : undefined,
    });
  }
  ...
  static captureError(
    ...
    Sentry.withScope((scope) => {
      if (context?.user) {
        SecuryFlexMonitoring.setUserContext(context.user);
      }
