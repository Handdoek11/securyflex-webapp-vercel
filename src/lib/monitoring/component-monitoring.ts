...
export function useComponentPerformance(componentName: string) {
  useEffect(() => {
    const transaction = SecuryFlexMonitoring.createTransaction(`Component: ${componentName}`, 'component')
...
export function useInteractionTracking(componentName: string) {
  const trackClick = useCallback((action: string, details?: Record<string, any>) => {
  ...
  const trackFormSubmit = useCallback((formType: string, success: boolean, errors?: string[]) => {
...
export function useZZPEventTracking() {
  const trackJobApplication = useCallback((jobId: string, success: boolean, error?: string) => {
    ...
    if (!success && error) {
      SecuryFlexMonitoring.trackSecurityEvent('application_failed', 'medium', {
        jobId,
        error,
  ...
  const trackShiftAction = useCallback((action: 'start' | 'pause' | 'resume' | 'end', shiftId: string, location?: { lat: number; lng: number }) => {
    ...
    if (location && (action === 'start' || action === 'end')) {
      SecuryFlexMonitoring.trackLocationEvent(`shift_${action}`, undefined, undefined)
    }
  }, [])

  const trackPaymentEvent = useCallback((event: 'request' | 'received' | 'disputed', amount: number, shiftId?: string) => {
    SecuryFlexMonitoring.trackPaymentEvent(`zzp_payment_${event}`, amount, 'EUR', {
      shiftId,
      timestamp: new Date().toISOString()
    })
  }, [])

  const trackDocumentUpload = useCallback((documentType: string, fileSize: number, success: boolean, error?: string) => {
    SecuryFlexMonitoring.trackFileUpload(documentType, fileSize, success, error)
...
export function useSecurityEventTracking() {
  const trackSuspiciousActivity = useCallback((activity: string, details?: Record<string, any>) => {
  ...
  const trackAuthenticationEvent = useCallback((event: 'login' | 'logout' | 'failed_login', details?: Record<string, any>) => {
    const eventMap = {
      'login': 'login_success',
      'logout': 'logout',
    ...
    SecuryFlexMonitoring.trackSecurityEvent(eventMap[event], event === 'failed_login' ? 'medium' : 'low', {
      timestamp: new Date().toISOString(),
      ...details
    })
  }, [])

  const trackDataAccess = useCallback((dataType: string, action: 'read' | 'write' | 'delete', authorized: boolean) => {
    if (!authorized) {
...
export function setupGlobalErrorHandling() {
  if (typeof window !== 'undefined') {
    ...
    window.addEventListener('unhandledrejection', (event) => {
      SecuryFlexMonitoring.captureError(new Error(`Unhandled Promise Rejection: ${event.reason}`), {
        tags: {
