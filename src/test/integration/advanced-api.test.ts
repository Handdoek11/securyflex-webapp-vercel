...
const mockPrisma = {
  ...
  opdracht: { findUnique: vi.fn(), findMany: vi.fn(), create: vi.fn(), update: vi.fn() },
  sollicitatie: { create: vi.fn(), findFirst: vi.fn(), findMany: vi.fn() },
  werkuren: { create: vi.fn(), findMany: vi.fn(), findUnique: vi.fn(), update: vi.fn() },
...
describe('Advanced API Integration Tests', () => {
  ...
  describe('Job Applications Flow', () => {
    ...
    describe('POST /api/jobs/[id]/apply', () => {
      it('should successfully apply ZZP user for job', async () => {
        ...
        const request = createMockRequest('POST', '/api/jobs/job-1/apply', applicationData)
        const response = await applyForJob(request, { params: { id: 'job-1' } })
  ...
  describe('Time Tracking Flow', () => {
    ...
    describe('POST /api/werkuren/clock-in', () => {
      it('should successfully clock in for assigned opdracht', async () => {
        ...
        const request = createMockRequest('POST', '/api/werkuren/clock-in', clockInData)
    ...
    describe('POST /api/werkuren/[id]/clock-out', () => {
      it('should successfully clock out with incident reports', async () => {
        ...
        const request = createMockRequest('POST', '/api/werkuren/werkuren-1/clock-out', clockOutData)
        const response = await clockOut(request, { params: { id: 'werkuren-1' } })
      ...
      it('should calculate accurate work hours and earnings', async () => {
        ...
        expect(responseData.data.netPay).toBeLessThan(responseData.data.brutoPay)
      })
    })
  })
  ...
  describe('Profile Management', () => {
    ...
    describe('PUT /api/profile', () => {
      ...
      it('should validate hourly rate limits', async () => {
        ...
        const request = createMockRequest('PUT', '/api/profile', invalidUpdateData)
        const response = await updateProfile(request)
  ...
  describe('Error Handling & Resilience', () => {
    it('should handle database connection failures gracefully', async () => {
      ...
      mockPrisma.user.findUnique.mockRejectedValue(createDatabaseError('connection'))
    ...
    it('should handle concurrent requests without data corruption', async () => {
      ...
      mockPrisma.user.findUnique.mockResolvedValue({ ...mockUser, zzpProfile: { id: 'zzp-1' } })
      mockPrisma.zZPProfile.update.mockResolvedValue({ id: 'zzp-1', uurtarief: 30.0 })

      // Simulate 10 concurrent profile updates
      const updates = Array.from({ length: 10 }, (_, i) =>
        updateProfile(createMockRequest('PUT', '/api/profile', { uurtarief: 25 + i }))
      )
  ...
  describe('Performance & Load Testing', () => {
    it('should handle rapid sequential requests efficiently', async () => {
      ...
      console.log(`Performance test: ${responses.length} requests in ${totalTime.toFixed(2)}ms (avg: ${avgResponseTime.toFixed(2)}ms)`)
    })
    ...
    it('should maintain response quality under load', async () => {
      ...
      mockPrisma.user.findUnique.mockResolvedValue({ zzpProfile: { id: 'zzp-1' } })
      mockPrisma.werkuren.findMany.mockResolvedValue(mockDatabaseResponses.sampleJobs)
      mockPrisma.opdrachtAssignment.findMany.mockResolvedValue([])
      mockPrisma.$transaction.mockResolvedValue([
