...
import {
  POST as createOpdracht,
  GET as getOpdrachten,
} from "@/app/api/opdrachten/route";
  ...
  describe("Jobs API", () => {
    describe("GET /api/jobs", () => {
      ...
      it("should apply search filters correctly", async () => {
        ...
        const request = createMockRequest(
          "GET",
          "/api/jobs?search=evenement&minRate=25&location=amsterdam",
        );
        const response = await getJobs(request);
    ...
    describe("POST /api/jobs", () => {
      ...
      it("should create job for authorized opdrachtgever", async () => {
        ...
        mockPrisma.opdrachtgever.findUnique.mockResolvedValue({
          id: "opdrachtgever-1",
        });
    ...
    describe("POST /api/opdrachten", () => {
      ...
      it("should reject opdracht creation for unauthorized users", async () => {
        ...
        expect(responseData.error).toContain(
          "Only opdrachtgevers and bedrijven",
        );
      });
    });
  });
...
function setupDatabaseMocks(scenario: "empty" | "populated" | "error") {
  switch (scenario) {
    ...
    case "populated":
      ...
      break;
    case "error":
      mockPrisma.opdracht.findMany.mockRejectedValue(
