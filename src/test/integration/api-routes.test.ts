...
describe('API Routes Integration Tests', () => {
  ...
  describe('Registration API', () => {
    ...
    it('should require all registration fields', async () => {
      ...
      const request = createRequest('POST', '/api/auth/register', incompleteData)
      const response = await POST(request)
  ...
  describe('Jobs API', () => {
    ...
    it('should return jobs for authenticated users', async () => {
      ...
      vi.mocked(mockPrisma.default.zZPProfile.findUnique).mockResolvedValue({ id: 'zzp-1' })

      // Mock database error to trigger fallback to mock data
      vi.mocked(mockPrisma.default.opdracht.findMany).mockRejectedValue(new Error('DB Error'))
    ...
    it('should validate query parameters', async () => {
      ...
      const request = createRequest('GET', '/api/jobs?page=2&limit=5&search=concert&minRate=30')
      const response = await GET(request)
  ...
  describe('Error Handling', () => {
    it('should handle database connection errors', async () => {
      ...
      const mockPrisma = await import('@/lib/prisma')
      vi.mocked(mockPrisma.default.zZPProfile.findUnique).mockRejectedValue(new Error('Connection failed'))
  ...
  describe('Performance & Reliability', () => {
    it('should handle concurrent requests', async () => {
      ...
      expect(vi.mocked(mockAuth.auth).mock.calls.length).toBeGreaterThanOrEqual(5)
    })
