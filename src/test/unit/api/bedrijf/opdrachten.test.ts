...
import { GET, POST } from '@/app/api/bedrijf/opdrachten/route';
...
describe('Bedrijf Opdrachten API', () => {
  ...
  describe('GET /api/bedrijf/opdrachten', () => {
    it('should return opdrachten for authenticated bedrijf', async () => {
      ...
      const { BedrijfQueryOptimizer } = await import('@/lib/database/optimization');
      const mockOptimizer = new BedrijfQueryOptimizer(mockPrisma);
      vi.mocked(mockOptimizer.getOpdrachtForBedrijf).mockResolvedValue(mockResult);

      const request = new NextRequest('http://localhost:3000/api/bedrijf/opdrachten?role=leverancier');
    ...
    it('should handle role filtering correctly', async () => {
      const request = new NextRequest('http://localhost:3000/api/bedrijf/opdrachten?role=opdrachtgever&status=OPEN');
    ...
    it('should handle pagination parameters', async () => {
      const request = new NextRequest('http://localhost:3000/api/bedrijf/opdrachten?page=2&limit=10');
    ...
    it('should reject unauthenticated requests', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/bedrijf/opdrachten');
  ...
  describe('Security and Performance', () => {
    it('should use query optimization', async () => {
      const { withDatabaseOptimization } = await import('@/lib/database/optimization');
      ...
      const optimizedFn = withDatabaseOptimization(mockOptimizeFn, 'test-cache-key');
