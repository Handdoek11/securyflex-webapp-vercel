...
export const mockUserProfiles = {
  ...
  bedrijf: {
    ...
    specialisaties: ['Object beveiliging', 'Evenement beveiliging', 'Mobiele surveillance'],
...
// Validation helpers
export function expectValidApiResponse(responseData: any, expectedProperties: string[] = []) {
...
export function expectValidJobData(job: any) {
  const requiredFields = [
    ...
    'startDate', 'endDate', 'hourlyRate', 'spotsAvailable',
    'spotsRemaining', 'status'
  ]
...
export function expectValidOpdrachtData(opdracht: any) {
  const requiredFields = [
    'id', 'titel', 'beschrijving', 'locatie', 'startDatum',
    'eindDatum', 'aantalBeveiligers', 'uurtarief', 'status'
  ]
...
export function createDatabaseError(type: 'connection' | 'constraint' | 'timeout' = 'connection') {
  const errors = {
    connection: new Error('Connection to database failed'),
    constraint: Object.assign(new Error('Unique constraint violation'), { code: 'P2002' }),
