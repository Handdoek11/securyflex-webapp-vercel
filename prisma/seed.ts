...
import { PrismaClient, UserRole, AccountStatus, OpdrachtStatus, CreatorType, TargetAudience, NDNummerStatus, CertificateStatus, DocumentStatus, SubscriptionTier } from '@prisma/client';
...
async function main() {
  ...
  const bedrijfProfile = await prisma.bedrijfProfile.create({
    data: {
      ...
      beschrijving: 'Professionele beveiligingsdiensten voor evenementen, bouw en horeca',
      werkgebied: ['Zuid-Holland', 'Noord-Holland', 'Utrecht'],
      specialisaties: ['Evenementen', 'Bouwbeveiliging', 'Horecabeveiliging', 'Objectbeveiliging'],
      ...
      ndNummerManagers: {
        managers: [
          { naam: 'Peter de Vries', functie: 'Directeur', ndNummer: 'ND789012-01' },
          { naam: 'Maria van der Berg', functie: 'Operationeel Manager', ndNummer: 'ND789012-02' }
        ]
      },
  ...
  const opdracht1 = await prisma.opdracht.create({
    data: {
      ...
      beschrijving: 'Beveiliging hoofdpodium tijdens driedaags zomerfestival. Crowd control, artiestenbegeleiding en VIP-beveiliging vereist.',
      startDatum: new Date('2024-07-05T08:00:00'),
      eindDatum: new Date('2024-07-07T23:00:00'),
      aantalBeveiligers: 12, // New field name
      uurtarief: 32.50, // New field name
  ...
  const opdracht2 = await prisma.opdracht.create({
    data: {
      ...
      beschrijving: 'Nachtbeveiliging voor grote bouwplaats. Toegangscontrole, rondes lopen en cameratoezicht.',
  ...
  await prisma.opdrachtSollicitatie.create({
    data: {
      ...
      zzpId: zzpProfile.id,
      status: 'PENDING',
      motivatie: 'Ruime ervaring met festivalbeveiliging. BOA gecertificeerd en bekend met crowd control.',
      voorgesteldTarief: 30.00,
