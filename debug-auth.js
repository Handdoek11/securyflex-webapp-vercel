...
async function debugAuth() {
  try {
    ...
    if (!user.emailVerified && user.status === "PENDING") {
      console.log('❌ Email not verified and status is PENDING, login would fail');
      return;
    }
    ...
    switch (user.role) {
      case 'ZZP_BEVEILIGER':
        hasCompletedProfile = !!(user.zzpProfile?.kvkNummer && user.zzpProfile?.uurtarief);
        break;
      case 'BEDRIJF':
        hasCompletedProfile = !!(user.bedrijfProfile?.bedrijfsnaam && user.bedrijfProfile?.kvkNummer);
        break;
      case 'OPDRACHTGEVER':
    ...
    console.log(`   Profile completed: ${hasCompletedProfile ? '✅ YES' : '❌ NO'}\n`);

    // Final result
    console.log('🎯 Final Assessment:');
    console.log(`   Login should: ${passwordValid && !isLocked ? '✅ SUCCEED' : '❌ FAIL'}`);
